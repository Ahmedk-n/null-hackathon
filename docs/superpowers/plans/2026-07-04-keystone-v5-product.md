...
- [x] `src/app/page.tsx` becomes the landing (server component shell + client hero); app moves to `src/app/studio/page.tsx` (same props: startedAt ISO from server, decision).
- [x] Sections, terminal/CAD aesthetic throughout (hairlines, zero radius, mono, paper):
...
- [x] Migrate: e2e/rehearsal.mjs (goto /studio), shell.test/route tests that assume /; T9 contract (TopBar ISO timestamp etc.) applies to the STUDIO page.
...
- [x] PRINT MEMO action in the studio TopBar (enabled when a verdict exists) → opens `/studio/memo` (client route reading the live store; if store empty → "NO ANALYSIS — RETURN TO STUDIO").
- [x] Sheet layout (print-optimized, @media print clean, A4): **title block** (bottom-right, CAD-style: decision title, ISO date from server-passed startedAt, scenario/source, INTEGRITY stamp with status word, SHEET 1/1, drawn-by KEYSTONE v0.1) · thesis + verdict banner · KNOCK-OUT SENSITIVITY table · DE-RISKING PLAN (minimal set, before→after) · CONSTRAINT REGISTER (planes + VIOLATED states) · EVIDENCE REGISTER (assumption → fact → source, provenance states) · FAILS-IN-N-DAYS line · attack ledger.
- [x] window.print() button on the sheet ("PRINT / SAVE AS PDF"). No new deps.
- Files: src/app/studio/memo/page.tsx + src/ui/memo/** (new), KeystoneApp TopBar button, tests (render with store populated + empty).

### V5-3 · Graph editing — inspector-panel CAD ✅ (rename/delete/addAssumption/flipGroupKind + editError chip; manual caps 3–25 via validateManualEdit; structural edits reset stress + rebuild from base; MODIFIED — UNVERIFIED provenance suppresses the evidence plate; 29 new tests. Deviations: one additive plumbing line in KeystoneCanvas; flipGroupKind also marks modified — consistent with the grilling rule)
- [x] Store actions (src/store/useKeystone.ts, additive): `renameNode(id,label)`, `setNodeConfidence` (exists as setConfidence — reuse), `deleteNode(id)` (cascade: remove from groups; dependents warning data), `addAssumption(parentClaimId,label,confidence)` (id slugified, unique), `flipGroupKind(nodeId,groupIndex)` (AND↔OR). EVERY structural action: (a) runs the edit on a clone, (b) re-validates with the validation-wall rules relaxed for manual edits (acyclic, one thesis, no orphans; node count 3–25), rejects invalid with a store-level `editError` surfaced as a chip, (c) RESETS the stress verdict (clear attacks/loadApplied/reinforcementPlan/timeline — back to baseline integrity), (d) marks the touched node `provenance: "modified"` → renders MODIFIED — UNVERIFIED (SelectionPanel + evidence plate detaches).
- [x] SelectionPanel EDIT section (studio GRAPH tab): rename field, confidence slider (exists), DELETE (with "N dependents will be orphaned→re-wired to parent" note), ADD ASSUMPTION (on claims), AND↔OR toggle per group. Terminal styling.
- [x] The solver re-verdicts live after every edit (integrity/keystone/sensitivity update immediately — that's the CAD moment).
- [x] Tests: each action (happy + rejected-invalid), stress-reset on structural edit, MODIFIED provenance, keystone recompute after edit.
...
- [x] Auto-save on every successful Analyse (and update on Apply Load/reinforce): snapshot {id, title=decision, startedAtISO (server-passed — NEVER client Date), mode/scenario, input, pack, graph, verdict summary {integrity, keystone, failed}} → localStorage (versioned key `keystone.library.v1`, capped 20 entries FIFO).
- [x] Studio: LIBRARY drawer/section (ContextTab rail or TopBar) — reopen (restores full snapshot into store + mode), duplicate, delete.
- [x] Landing: DECISIONS ledger section lists recent 5 (title, integrity stamp, keystone, date) — click → /studio?open=<id> restores it.
- [x] Client-timestamp rule: use the server-rendered startedAt prop + a per-session monotonic counter for ordering; no Date.now()/new Date( in client files (T8 guard stays green).
- [x] Tests: save/restore round-trip, cap/FIFO, corrupted-entry tolerance (bad JSON → skipped, never crashes).
