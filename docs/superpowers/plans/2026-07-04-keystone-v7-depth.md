...
- [x] **BUG 2 SelectionPanel** confidence rows (`SelectionPanel.tsx`): covered by the root fix; verify the `— UNVERIFIED/ASSUMED` suffix doesn't overflow the 300px rail (move to a sub-line if needed).
...
- [x] **Attack rationale** in `AttackRow` (`StressTab.tsx`): render `attack.rationale` + a target LABEL (not just id) under the severity — the specific "why it breaks". Biggest single win.
- [x] **Load-result summary** panel via `summariseLoadResult`: `baseline → post-load (−drop)`, failed-node labels, `keystone before → after` shift. Gated on `loadApplied`.
- [x] **Keystone explanation** via `explainKeystone`: the deterministic sentence + `impactRatio` ("30× more load-bearing than the next assumption") on/above the sensitivity bars.
- [x] **Support breakdown** via `supportBreakdown`: "why integrity is that number" — per-node `ownConfidence × dependencyFactor = support`, failed flagged (collapsible / right rail; reuse the LivePipeline treatment).
- [x] **Firm-up payoff** (small new pure fn in `engine`, e.g. `marginalReinforcement`): per assumption, integrity gain from restoring its base confidence on the attacked graph → a `+N integrity` number on each DE-RISKING `PROVE ·` row. Pure + tested.
...
- [x] **Feed the full pack** into extract + attack prompts: `renderPack` (`client.ts`) currently drops `relevantConstraints/relevantObjectives/relevantKnownRisks/missingInformation` — render them so the model extracts against real constraints/risks. Live path; engine-inert.
- [x] **Multi-citation evidence**: `node.evidence` single → `evidence[]` (supporting + contradicting), so the ~12 discarded findings surface. `EvidenceSchema`→array, `NodeEvidence`, extract prompt cites multiple, `validate.ts` clone passes it through, evidence-plate UI renders 1-2 (engine-inert — no integrity change). Re-pin fixture evidence shape + SelectionPanel/plate tests.
...
- [x] **Deepen gather loops** (latency-aware): raise `technical max_iterations` and `business max_uses` modestly so agents dig into multiple files/sources — keep within the 30-60s timeout envelope (comment the trade-off; fixture fallback still protects the demo).
