...
describe("applyAttacks", () => {
  ...
  it("ignores attacks on unknown ids", () => {
    const out = applyAttacks(graph(), [{ id: "x", targetId: "ghost", category: "m", severity: 0.9, rationale: "" }]);
    expect(out.nodes.find((n) => n.id === "a1")!.confidence).toBe(0.8);
  });
...
describe("detectFailures", () => {
  it("flags nodes whose support falls below the default threshold", () => {
  ...
  it("respects a custom threshold", () => {
    // baseline a1 = 0.8, T = 0.8; with threshold 0.9 both are "failed"
    const failures = detectFailures(graph(), 0.9);
    expect(failures.has("a1")).toBe(true);
    expect(failures.has("T")).toBe(true);
    // with default threshold, nothing fails
    expect(detectFailures(graph()).size).toBe(0);
  });
});
